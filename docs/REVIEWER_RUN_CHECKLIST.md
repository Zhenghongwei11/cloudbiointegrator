...
### Cross-Environment Verification (recommended, free)

The manuscript's reproducibility claim is backed by **run-scoped outcome summaries**
(`results/runs/<run_id>/<stage>_summary.json`) whose SHA-256 are compared between a local
Docker run and a clean GitHub Actions runner. See `docs/CROSS_ENV_CI_CHECK.md` for the
one-time local baseline step; after that, every push to `main` runs the check automatically.

```bash
# Record the latest local run-scoped manifest
make manifest

# Compare two manifests manually (e.g., local vs CI artifact)
make compare-manifest ARGS="results/runs/baseline_manifest.tsv results/runs/ci_manifest.tsv"
```

The 84-row `results/audit/reproducibility_checks.tsv` is a **run-level log** (each row records
the SHA-256 of a cumulative output table at write time); it is traceability bookkeeping, not a
cross-environment comparison. Cross-environment byte-level claims must cite the run-scoped
manifest comparison above.
