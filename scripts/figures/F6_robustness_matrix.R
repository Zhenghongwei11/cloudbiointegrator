...
out_pdf <- file.path(outdir, "pdf", "F6_perturbation_sensitivity.pdf")
out_png <- file.path(outdir, "png", "F6_perturbation_sensitivity.png")
...
n_eval_total <- sum(df$pass %in% c(0, 1), na.rm = TRUE)
...
if (nrow(fail_df) > 0) {
  ...
  fail_df$failure_reason <- gsub("hvg_half", "HVG/2", fail_df$failure_reason)
  fail_df$failure_reason <- gsub("seed_plus_1", "seed+1", fail_df$failure_reason)
  fail_df$failure_reason <- gsub("ARI<0.90", "ARI < 0.90", fail_df$failure_reason)
  fail_df$failure_reason <- gsub("Pearson<0.90", "Pearson < 0.90", fail_df$failure_reason)
