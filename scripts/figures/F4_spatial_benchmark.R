...
# Per-method robust color scaling: normalize each method's weights to its own
# inter-quantile range (q05-q95) so smooth, low-magnitude methods (e.g.,
# cell2location) show spatial gradients without saturating the color scale.
method_q5 <- stats::aggregate(weight ~ method, data = maps, FUN = function(x) {
  x <- x[x > 0]
  as.numeric(stats::quantile(x, 0.05, na.rm = TRUE))
})
method_q95 <- stats::aggregate(weight ~ method, data = maps, FUN = function(x) {
  x <- x[x > 0]
  as.numeric(stats::quantile(x, 0.95, na.rm = TRUE))
})
maps <- merge(maps, method_q5, by = "method", suffixes = c("", "_q5"))
maps <- merge(maps, method_q95, by = "method", suffixes = c("", "_q95"))
spread <- maps$weight_q95 - maps$weight_q5
spread[!is.finite(spread) | spread <= 0] <- 1
maps$weight_scaled <- pmin(pmax((maps$weight - maps$weight_q5) / spread, 0), 1)
hi <- 1

p_maps <- ggplot(maps, aes(x = array_col, y = array_row, color = weight_scaled)) +
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      "L1-normalized per-spot compositions"
    ),
    color = "Weight"
  ) +
  plot_map_theme
...
if (!is.null(weights_cell2_n)) {
  cos_rc <- spotwise_cosine(weights_rctd_n, weights_cell2_n, common_ct)
  if (!is.null(cos_rc)) conc_rows[[length(conc_rows) + 1]] <- data.frame(pair = "RCTD vs cell2location", cosine = cos_rc)
}
if (!is.null(weights_cell2_n)) {
