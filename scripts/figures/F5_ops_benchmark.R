...
out_pdf <- file.path(outdir, "pdf", "F5_operational_feasibility.pdf")
out_png <- file.path(outdir, "png", "F5_operational_feasibility.png")
...
df$failure_type <- tolower(df$failure_type)
df$failure_type <- gsub("_", " ", df$failure_type)
df$failure_type[grepl("runtime ?error", df$failure_type)] <- "Runtime error"
df$failure_type[grepl("missing ?dependency", df$failure_type)] <- "Missing dependency"
df$failure_type <- gsub("_", " ", df$failure_type)
...
base_theme_xrot <- base_theme + theme(axis.text.x = element_text(angle = 40, hjust = 1, size = 8.1))
