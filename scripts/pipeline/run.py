...
def write_run_summary(
    *,
    ...
    """Write a deterministic, run-scoped outcome summary for cross-environment verification.

    The JSON payload intentionally excludes timestamps, wall-clock times, run ids, and other
    environment-dependent fields, so that two runs of the same stage + dataset under the same
    pinned runtime produce byte-identical files whose SHA-256 can be compared across
    environments (e.g., local Docker vs GitHub Actions).
    """
    payload = {
        "stage": stage,
        "dataset_id": dataset_id,
    ...
    path = ROOT / "results" / "runs" / run_id / f"{stage}_summary.json"
    ensure_parents(path)
    path.write_text(
...
def cmd_skeleton(_: argparse.Namespace) -> int:
    ...
    write_run_summary(
        ...
        dataset_id="NA",
        summary={
            "action_schema_version": schema.get("schema_version", ""),
            "n_allowed_actions": len(schema.get("allowed_actions", [])),
            "required_contract_tables": [str(p) for p in REQUIRED_TABLES],
        },
    )
    ...
    try:
        ...
        write_run_summary(
            ...
            summary={
                "n_cells_or_spots": stats["n_cells_or_spots"],
                "n_genes": stats["n_genes"],
                "matrix_n_rows": stats["matrix_n_rows"],
                "matrix_n_cols": stats["matrix_n_cols"],
                "matrix_n_entries": stats["matrix_n_entries"],
                "seed": seed,
            },
        )
...
def cmd_visium(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.method_pack == "baseline":
            ...
            write_run_summary(
                ...
                summary={
                    ...
                    "params": {"neighbors": 6, "leiden_res": 0.5},
                },
            )
        else:
        ...
        write_repro_check(
            ...
            env_hash=env_hash,
            seed=seed,
            output_table_path="results/benchmarks/runtime_cost_failure.tsv",
            notes=f"FAILED run; {type(e).__name__}",
            pass_flag=False,
            fail_reason=failure_type,
            wall_time_s=wall,
        )
        raise
        ...
        if len(input_dirs) >= 2:
            ...
            write_run_summary(
                ...
                summary={
                    ...
                    "compute_tier": compute_tier,
                    "seed": seed,
                    "n_batches": out_int["n_batches"],
                    "n_cells_after_qc": out_int["n_cells_after_qc"],
                    "n_genes_after_filter": out_int["n_genes_after_filter"],
                    "n_clusters_baseline": out_int["n_clusters_baseline"],
                    "n_clusters_harmony": out_int["n_clusters_harmony"],
                    "batch_mixing_baseline_mean": out_int["batch_mixing_baseline_mean"],
                    "batch_mixing_harmony_mean": out_int["batch_mixing_harmony_mean"],
                    "batch_mixing_scvi_mean": out_int.get("batch_mixing_scvi_mean"),
                    "label_purity_baseline_mean": out_int.get("label_purity_baseline_mean"),
                    "label_purity_harmony_mean": out_int.get("label_purity_harmony_mean"),
                    "ari_clusters_baseline_vs_harmony": out_int["ari_clusters_baseline_vs_harmony"],
                    "ari_clusters_scvi_vs_harmony": out_int.get("ari_clusters_scvi_vs_harmony"),
                    "ari_clusters_scvi_vs_scvi_hvg_half": out_int.get("ari_clusters_scvi_vs_scvi_hvg_half"),
                    "scvi_params": {
                        "n_hvg": scvi_n_hvg,
                        "n_latent": scvi_n_latent,
                        "max_epochs": scvi_max_epochs,
                        "max_cells": scvi_max_cells,
                    },
                    "versions": out_int["versions"],
                    "notes": out_int["notes"],
                },
            )
        else:
            ...
            write_run_summary(
                ...
                summary={
                    ...
                    "concordance": out.get("concordance") or {},
                    "annotation": out.get("annotation") or {},
